...
void TernaryCanvas::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->button() == Qt::LeftButton && compositionForPoint(event->position(), &a, &b, &c)) {
        ...
        if (interaction_mode_ == 2) emit interpolationRequested(a, b, c);
...
void TernaryCanvas::mouseDoubleClickEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton && interaction_mode_ == 1) {
...
void TernaryCanvas::paintEvent(QPaintEvent*) {
    ...
    if (plot_visible_) {
        for (const auto& path : projection_paths_) {
            if (path.compositions.size() < 2) continue;
            ...
            const QColor color = path.type == 0 ? QColor(45, 110, 180) : path.type == 1 ? QColor(160, 55, 130) : QColor(180, 55, 55);
            painter.setPen(QPen(color, path.type == 0 ? 1.5 : 2.2)); painter.setBrush(Qt::NoBrush); painter.drawPath(curve);
        }
    ...
    if (source_vertices_visible_) {
        for (const auto& vertex : inspection_vertices_) {
            ...
            if (!vertex.label.isEmpty() && (selected || marker_size_ >= 7)) { painter.setPen(palette().text().color()); painter.drawText(point + QPointF(marker_size_ + 2, -2), vertex.label); }
        }
    }
